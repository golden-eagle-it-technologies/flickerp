...
from task.models import Attachment, Comment, Task

log = logging.getLogger(__name__)


def staff_check(user):
    """If task_STAFF_ONLY is set to True, limit view access to staff users only.
        # FIXME: More granular access control needed - see
        https://github.com/shacker/django-task/issues/50
    """

    if defaults("task_STAFF_ONLY"):
...
def task_get_backend(task):
    """Returns a mail backend for some task"""
    mail_backends = getattr(settings, "task_MAIL_BACKENDS", None)
...
def task_get_mailer(user, task):
    """A mailer is a (from_address, backend) pair"""
    task_backend = task_get_backend(task)
    if task_backend is None:
...
def task_send_mail(user, task, subject, body, recip_list):
    ...
    from_address, backend = task_get_mailer(user, task)
    ...
    thread_message_id = "<thread-{}@django-task>".format(task.pk)
...
def send_notify_mail(new_task):
    ...
    subject = render_to_string("task/email/assigned_subject.txt", {"task": new_task})
    body = render_to_string(
        "task/email/assigned_body.txt", {"task": new_task, "site": current_site}
    )

    recip_list = [new_task.assigned_to.email]
    task_send_mail(new_task.created_by, new_task, subject, body, recip_list)
...
def send_email_to_thread_participants(task, msg_body, user, subject=None):
    ...
    if not subject:
        subject = render_to_string("task/email/assigned_subject.txt", {"task": task})

    email_body = render_to_string(
        "task/email/newcomment_body.txt",
    ...
    task_send_mail(user, task, email_subject, email_body, recip_list)
