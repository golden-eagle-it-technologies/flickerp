...
from task.models import Comment, Task, TaskList
...
def message_text(message):
    ...
    # task: find something smart to do when no text if found
    return ""
...
DJANGO_task_THREAD = re.compile(r"<thread-(\d+)@django-task>")
...
def parse_references(task_list, references):
    ...
    for related_message in references.split():
        ...
        match = re.match(DJANGO_task_THREAD, related_message)
        if match is None:
...
def insert_message(task_list, message, priority, task_title_format):
    ...
    # (generated by django-task) could be found, use it
