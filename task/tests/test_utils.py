...
from task.utils import send_email_to_thread_participants, send_notify_mail


def test_send_notify_mail_not_me(task_setup, django_user_model, email_backend_setup):
    """Assign a task to someone else, mail should be sent.
    task: Future tests could check for email contents.
    """
...
def test_send_notify_mail_myself(task_setup, django_user_model, email_backend_setup):
...
def test_send_email_to_thread_participants(task_setup, django_user_model, email_backend_setup):
...
def test_defaults(settings):
    """task's `defaults` module provides reasonable default values for unspecified settings.
    If a value is NOT set, it should be pulled from the hash in defaults.py.
    If a value IS set, it should be respected.
    n.b. task_STAFF_ONLY which defaults to True in the `defaults` module."""
