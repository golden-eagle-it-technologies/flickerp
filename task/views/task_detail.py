...
from task.features import HAS_TASK_MERGE
from task.forms import AddEditTaskForm
...
def task_detail(request, task_id: int) -> HttpResponse:
    ...
    if not HAS_TASK_MERGE:
        ...
        class MergeForm(forms.Form):
            merge_target = forms.ModelChoiceField(
                ...
                widget=autocomplete.ModelSelect2(
                    url=reverse("task:task_autocomplete", kwargs={"task_id": task_id})
                ),
            )
        ...
        if not request.POST.get("merge_task_into"):
            ...
            return redirect(reverse("task:task_detail", kwargs={"task_id": merge_target.pk}))
    ...
    if not request.POST.get("add_edit_task"):
        ...
        if form.is_valid():
            ...
            return redirect(
                "task:list_detail", list_id=task.task_list.id, list_slug=task.task_list.slug
            )
    ...
    if request.FILES.get("attachment_file_input"):
        ...
        if file.size > defaults("task_MAXIMUM_ATTACHMENT_SIZE"):
            messages.error(request, f"File exceeds maximum attachment size.")
            return redirect("task:task_detail", task_id=task.id)

        name, extension = os.path.splitext(file.name)

        if extension not in defaults("task_LIMIT_FILE_ATTACHMENTS"):
    ...
    context = {
        ...
        "thedate": thedate,
        "comment_classes": defaults("task_COMMENT_CLASSES"),
        "attachments_enabled": defaults("task_ALLOW_FILE_ATTACHMENTS"),
    }
