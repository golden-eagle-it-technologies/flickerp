...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if not hasattr(settings, "task_MAIL_TRACKERS"):
            logger.error("missing task_MAIL_TRACKERS setting")
            sys.exit(1)

        worker_name = options["worker_name"]
        tracker = settings.task_MAIL_TRACKERS.get(worker_name, None)
        if tracker is None:
            logger.error("couldn't find configuration for %r in task_MAIL_TRACKERS", worker_name)
            sys.exit(1)
