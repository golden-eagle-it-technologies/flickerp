...
from organisation_details.utils import get_all_departments, get_department, get_position, get_all_positions, get_all_teams, get_team, get_team_employees, get_department_employees
from fauth.decorators import hr_required, ems_login_required
from organisation_details.models import Position, Department, Team

from config.utils import get_all_currencies, get_currency
...
class DepartmentsListView(generic.ListView):
    model = Department
    template_name = 'organisation_details/manage_departments.html'
    def get_context_data(self, **kwargs):
        context = super(DepartmentsListView, self).get_context_data(**kwargs)
        context["employees"] = get_active_employees()
        return context
...
def edit_job_position(request):
    ...
    position.salary_scale = request.POST.get('salary')
